...
class FinancialKPIs:
    ...
    def monthly_kpis(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        revenue = grouped.apply(lambda x: x[x["amount"] > 0]["amount"].sum(), include_groups=False)

        # Expenses = sum of absolute negative amounts
        expenses = grouped.apply(lambda x: -x[x["amount"] < 0]["amount"].sum(), include_groups=False)
