...
def generate_cfo_pdf_report(
    ...
    # Add tables
    add_table(pdf, "Monthly KPIs", kpis.reset_index())
    add_table(pdf, "Profit & Loss", pnl.reset_index())
    add_table(pdf, "Variance Analysis", variance.reset_index())

    runway_table = runway[["ds", "cash_balance"]].copy()
    runway_table["ds"] = runway_table["ds"].dt.date
